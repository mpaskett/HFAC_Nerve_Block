...
ts = [0 50];
T_start = [0 50];
I_step = .1e4;     % "synaptic" input to initiate action potential (fA)
...
freq = 04;          % kHz
amp  = 0.052e-3;     % amplitude of input current for blocking stimulus(A)
a    = 1;           % axon radius (um)
K_array = [10];     % set to 10 for myelinated, 100 for unmyelinated
...
[t,Y] = ode15s(@(t,Y) HH_axon_myelin_2(t,Y,@stepCurrentChuck,{T_start, I_step},freq,amp,a,Lm), ts, y0, ode_opts);
